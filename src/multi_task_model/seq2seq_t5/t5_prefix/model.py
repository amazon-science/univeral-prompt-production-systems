...
from src.prompts.prompt_generators import LinearPrompt


@register_model
class _T5_Prefix(T5ForConditionalGeneration):
    """
    _T5_Prefix is very similar to `T5ForConditionalGeneration` with a few differences to
    handle the field `descriptors` in the methods below.
    """
    def __init__(self, config, **kwargs):
        super(_T5_Prefix, self).__init__(config)
        prompt_generator = LinearPrompt(config)
