...
from src.prompts.prompt_generators import UniversalPromptProductionSystem


@register_model
class _T5_Prefix_UniProdSys(T5ForConditionalGeneration):
    """
    _T5_Prefix_UniProdSys is very similar to `T5ForConditionalGeneration` with a few differences to
    handle the field `descriptors` in the methods below.
    """
    def __init__(self, config, **kwargs):
        super(_T5_Prefix_UniProdSys, self).__init__(config)
        prompt_generator = UniversalPromptProductionSystem(
            config,
    ...
    @staticmethod
    def model_type():
        return "BART_PREFIX_UNIVERSAL_PRODUCTION_SYSTEM"
