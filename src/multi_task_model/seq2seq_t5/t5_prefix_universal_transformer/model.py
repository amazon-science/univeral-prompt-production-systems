...
from src.prompts.prompt_generators import UniversalTransformerPrompt


@register_model
class _T5_Prefix_UniTrsf(T5ForConditionalGeneration):
    """
    _T5_Prefix_UniTrsf is very similar to `BartForConditionalGeneration` with a few differences to
    handle the field `descriptors` in the methods below.
    """
    def __init__(self, config, **kwargs):
        super(_T5_Prefix_UniTrsf, self).__init__(config)
        prompt_generator = UniversalTransformerPrompt(
            config,
    ...
    @staticmethod
    def model_type():
        return "BART_PREFIX_UNIVERSAL_TRANSFORMER"
