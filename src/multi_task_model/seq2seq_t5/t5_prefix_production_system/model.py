...
from src.prompts.prompt_generators import PromptProductionSystem


@register_model
class _T5_Prefix_ProdSys(T5ForConditionalGeneration):
    """
    _T5_Prefix_ProdSys is very similar to `T5ForConditionalGeneration` with a few differences to
    handle the field `descriptors` in the methods below.
    """
    def __init__(self, config, **kwargs):
        super(_T5_Prefix_ProdSys, self).__init__(config)
        prompt_generator = PromptProductionSystem(
            config,
            token_embeddings=self.model.shared,
        )
        self.model = MyT5Model(config, prompt_generator)
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        if "decoder_start_token_id" in model_kwargs:  # new
            decoder_start_token_id = model_kwargs["decoder_start_token_id"]
            model_kwargs["decoder_start_token_id"] = decoder_start_token_id.index_select(0, expanded_return_idx)
    ...
    @staticmethod
    def model_type():
        return "BART_PREFIX_PRODUCTION_SYSTEM"
        ...
        **kwargs,
        ...
        if labels is not None:
            if decoder_input_ids is None:
                decoder_input_ids = modeling_t5.shift_tokens_right(
